...
CREATE TABLE IF NOT EXISTS `clinica`.`medico` (
  ...
  `crm_medico` VARCHAR(8) NOT NULL COMMENT 'CRM do Médico\nMáximo 8 números',
...
CREATE TABLE IF NOT EXISTS `clinica`.`consulta` (
  ...
  CONSTRAINT `fk_consulta_medico1`
    ...
    REFERENCES `clinica`.`medico` (`id_medico` , `especialidade_id_especialidade`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
-- -----------------------------------------------------
-- Table `clinica`.`medico_has_especialidade`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `clinica`.`medico_has_especialidade` (
  `especialidade_id_especialidade` INT NOT NULL,
